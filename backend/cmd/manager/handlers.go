...
func (h *Handler) SearchMarkets(c *gin.Context) {
	...
	// pgvector uses the <-> operator for L2 distance (or <=> for cosine distance).
	// We use the pgvector.NewVector helper to safely format the vector for the query.
	// However, gorm can pass []float32 via pgvector-go if registered properly,
	// or we can use the string representation '[1.0, 2.0, ...]' which pgvector accepts.

	// Ensure queryVec is []float32
	// Construct the string format '[x,y,z]' for Postgres
	queryVecString = pgVectorString(queryVec)
